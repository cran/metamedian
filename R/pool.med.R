...
#'
#' This function meta-analyzes the study-specific effect sizes by applying the (weighted) median of medians method (McGrath et al., in press) in one-sample contexts and the (weighted) median of the difference of median method (McGrath et al., 2018) in two-sample contexts.
#'
#' For one-group studies, authors may report the sample median or mean. If these measures are supplied for \code{yi} and weights are not provided for \code{wi}, the function implements the median of medians (MM) method (McGrath et al., in press).
#'
...
#'
#' @references McGrath S., Zhao X., Qin Z.Z., Steele R., and Benedetti A. (in press). One-sample aggregate data meta-analysis of medians. \emph{Statistics in Medicine}. \url{https://arxiv.org/abs/1709.03016}.
